...
namespace Editors.Shop
{
    public class CIShopGood
    {
        ...
        public CIShopGood(RefShopGood good)
        {
            ...
            {
                DataRow[] allPolicy = ClientDataStorage.Database.SRO_VT_SHARD.dbo.Tables["_RefPricePolicyOfItem"].Rows.OfType<DataRow>().Where(packItem => packItem.Field<string>("RefPackageItemCodeName") == PackageItemCodeName && packItem.Field<byte>("Service") >= 1).ToArray();
