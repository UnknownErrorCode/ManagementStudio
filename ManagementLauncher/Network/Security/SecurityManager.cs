                    ...
                    foreach (TransferBuffer buffer in incoming_buffers_tmp)
                    {
                        ...
                        if (m_client_security)
                        {
                            if (m_security_flags.security_bytes == 1)
                            {
                                ...
                                {
                                    throw (new Exception($"[SecurityAPI::Recv] Count byte mismatch on {packet_opcode.ToString("X4")}. (Expected: {expected_count}, Packet: {packet_security_count})"));
                                ...
                                {
                                    throw (new SecurityException($"SecurityManager::Recv->CRC byte mismatch on {packet_opcode.ToString("X4")}.(Expected: {expected_crc}, Packet: {packet_security_crc})"));
                        ...
                        else
                        {
                            ...
                            if (m_identity_flag == 1 && packet_opcode == 0x2001 && m_identity_name != "Tool_Client" && m_identity_name != "Tool_Launcher")
                            {
