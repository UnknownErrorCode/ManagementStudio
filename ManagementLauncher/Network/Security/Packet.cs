    ...
    public class Packet : IDisposable
    {
        ...
        public DataTable ReadDataTable(byte[] array)
        {
            lock (_lock)
            {
                ...
                DataTable vvalue = new DataTable();
        ...
        public string ReadAscii(int int1, int int2)
        {
            lock (_lock)
            {
                if (!_locked)
        ...
        public PaddedString_256 ReadPaddernString256()
        {
            lock (_lock)
            {
                if (!_locked)
                ...
                PaddedString_256 paddern = new PaddedString_256(ssssss);

                return paddern;
            }
        }
        public PaddedString_64 ReadPaddernString64()
        {
            lock (_lock)
            {
                if (!_locked)
                ...
                PaddedString_64 paddern = new PaddedString_64(ssssss);

                return paddern;
            }
        }
        public PaddedString_32 ReadPaddernString32()
        {
            lock (_lock)
            {
                if (!_locked)
                ...
                PaddedString_32 paddern = new PaddedString_32(ssssss);

                return paddern;
            }
        }
        public PaddedString_16 ReadPaddernString16()
        {
            lock (_lock)
            {
                if (!_locked)
                ...
                PaddedString_16 paddern = new PaddedString_16(ssssss);
        ...
        public void WriteChars(string value, int length)
        {
            lock (_lock)
            {
                if (_locked)
