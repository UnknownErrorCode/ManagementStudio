...
namespace WorldMapSpawnEditor.MapGame
{
    class MapCamera
    {
        ...
        public MapCamera(float AspectRotation)
        {
            AspectRatio = AspectRotation;
            Position = new Vector3(0.0f, 0.0f, 0.3f);
        }
        ...
        {
            if (e.KeyCode == Keys.S)
                Position -= _front * speed; //Backwards
            else if (e.KeyCode == Keys.W)
                Position += _front * speed; //Forward 
            else if (e.KeyCode == Keys.A)
                Position -= Vector3.Normalize(Vector3.Cross(_front, _up)) * speed; //Left
            else if (e.KeyCode == Keys.D)
                Position += Vector3.Normalize(Vector3.Cross(_front, _up)) * speed; //Right
            else if (e.KeyCode == Keys.Space)
                Position += _up * speed; //Up 
            else if (e.KeyCode == Keys.Y)
                Position -= _up * speed; //Down
