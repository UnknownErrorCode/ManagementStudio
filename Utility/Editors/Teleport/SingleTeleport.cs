...
namespace Editors.Teleport
{
    public class SingleTeleport
    {
        ...
        private void InitializeTeleport()
        {
            ...
            {
                if (PluginFramework.Database.SRO_VT_SHARD._RefObjCommon.TryGetValue(Teleport.AssocRefObjID, out RefObjCommon objCommon))
                {
                    ObjCommon = objCommon;
                    IsNpcTeleport = (ObjCommon.TypeID1 == 1 && ObjCommon.TypeID2 == 2 && ObjCommon.TypeID3 == 2 && ObjCommon.TypeID4 == 0);
                }
            }
            for (int i = 0; i < PluginFramework.Database.SRO_VT_SHARD._RefTeleLink.Count; i++)
            {
                if (PluginFramework.Database.SRO_VT_SHARD._RefTeleLink[i].OwnerTeleport.Equals(Teleport.ID)
                    || PluginFramework.Database.SRO_VT_SHARD._RefTeleLink[i].TargetTeleport.Equals(Teleport.ID))
                {
                    TeleLinks.Add(PluginFramework.Database.SRO_VT_SHARD._RefTeleLink[i]);
                }
            }
        }
    }
